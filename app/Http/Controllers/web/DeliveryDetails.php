...
class DeliveryDetails extends Controller
{
    public function index($deliveryId) {
    	...
    	$store_products = DB::select('select products_categories.name as p_c_name , products.* from products LEFT JOIN products_categories ON products.product_category_id = products_categories.id where store_id = "'.$deliveryId.'" AND status = 1 ORDER BY regular_price ASC ');
    	$store_id = $deliveryId;
    	$products_data = "";
        $logged_in_user = "";
        $logged_in_user_get = "";
