...
class DoctorsDetails extends Controller
{
    public function index($deliveryId) {
        ...
        $delivery_detail = DB::select('select * from stores where category = "1" AND id = "'.$deliveryId.'"  ORDER BY id DESC ');
        ...
        return view("web/doctorDetails", compact("delivery_detail","store_products","store_id","products_data","products_categories","sotore_folowing","your_review","redirect_controller_name","weighted_review_rating", "store_deals", "store_deals","store_media","able_to_rate_store_message","able_to_rate_store","product_page_menus","store_amenity"));
    }
}